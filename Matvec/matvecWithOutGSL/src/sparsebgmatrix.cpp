...
extern void odrvd(int n,int *ia,int *ja,BG *a,int *p,int *ip,
           int nsp,int *isp,int path,int& flag);
extern int sdrvd(int n,int *p,int *ip,int *ia,int *ja,BG *a,
           const BG *b,BG *z,int nsp,int *isp,BG *rsp,BG& esp,
           int path,int& flag,BG tol,int& rank,BG& lgdet);
extern void detdrv(int n,int *p,int *ip,int *ia,int *ja,BG *a,
                int nsp,int *isp,BG *rsp,BG& esp,int path,
                int& flag,double tol,int& rank,BG& lgdet);
*/
extern void gs_ioc(int *ia,int *ja,BG *a,const BG *rhs,BG *sol,
...
// extern BG snorm(void);

static int squeeze(unsigned len,int *iiv,int *jjv,BG *aav);
static void hsort_ija(unsigned n,int *iiv,int *jjv,BG *aav);

void gs_ioc(int *ia,int *ja,BG *a,const BG *rhs,BG *sol,const int n,
...
{
   int j,row,col;
   BG oldsol,newsol,local,diag, cmax,cval;
   Vector<BG> adj_rhs(n);
   int niter = 0;
   cov = 0;
 ...
 */
SparseBGMatrix::SparseBGMatrix(void)
{
   dim         = 0;
   nonzero     = 0;
   max_nz      = 0;
 ...
 */
SparseBGMatrix::SparseBGMatrix(const SparseBGMatrix& A)
{
   dim         = 0;
   nonzero     = 0;
   max_nz      = 0;
...
/**
 * A constructor to create an SparseBGMatrix object of size
 *  n by n and maximum non-zero elemenat maxnz.
 */
SparseBGMatrix::SparseBGMatrix(int n,int maxnz)
{
   if (n<0 || maxnz<0) throw exception("SparseBGMatrix(m,n): requring nonnegative m,n");
   dim = n;
   max_nz = maxnz;
   nonzero = 0;
   factor_done = 0;
   ...
   // memset(aav,'\0',sizeof(BG)*hsize);
   initial_BGarray(aav,hsize,0.0);
   if(hsize){
   ...
   nonzero=0;
   BG temp(0,0.0);
   for (unsigned i=1; i<=dim; i++) insert(i,i,temp);
}

/**
 * Get a copy from A
 */
void SparseBGMatrix::copyfrom(const SparseBGMatrix& A)
{
   if (this == &A) return;
   ...
   memcpy(aav,A.aav,sizeof(BG)*hsize);
   if(hsize){
 ...
 */
BG SparseBGMatrix::operator()(const int i,const int j) {

  if (i>=1 && i<=dim && j>=1 && j<=dim) {
    return getaij(i,j);
  }
  else {
    throw exception("SparseBGMatrix(): subscript out of range");
  }
  return BG(0,0.0);
}
 ...
 * Assignment operator.
 */
const SparseBGMatrix& SparseBGMatrix::operator=(const SparseBGMatrix& A)
{
  copyfrom(A);
  return *this;
}
 ...
 */
SparseBGMatrix& SparseBGMatrix::resize(const int n,const int maxnz)
{
  // std::cout << "\n n " << n << " dim " << dim << " max_nz " << max_nz << " maxnz " << maxnz <<"\n";
   ...
   for (unsigned i=1; i<=dim; i++) insert(i,i,BG(0,0.0));
   return *this;
}
 ...
 */
Vector<BG> SparseBGMatrix::row(const int ithrow)
{
   if ( dim == 0 ) throw exception("SparseBGMatrix is empty");
   if (ithrow<1 || ithrow>dim) throw exception("SparseBGMatrix.row(k): out of range");
   Vector<BG> out(dim);
   int *ia = iiv-1;
   int *ja = jjv-1;
   BG *a = aav-1;
...
{
  if ( A.dim == 0) throw exception("SparseBGMatrix is empty");
  if (A.dim != v.size()) throw exception("SparseBGMatrix*Vector<BG>, size not conformable");
  Vector<BG> vec_out;
  int n = A.dim;
...
void SparseBGMatrix::mv(const Vector<BG> &v, Vector<BG> &result)
{
   if (dim != v.size()) throw exception("SparseBGMatrix::mv(): sizenot conformable");
   if (v.begin() == result.begin()) throw exception("SparseBGMatrix::mv(): can't be done in situ");
   result.reserve(dim);
   this->mv(v.begin(),dim,result.begin());
}

void SparseBGMatrix::mv(const BG *v, const unsigned n, BG *result) {
   /////////////////////////////////////////////////////////////////////
   // REQUIREMENTS:  both v and result must have n(dim) elements (space)
   // modified 8/4/1998
   //////////////////////////////////////////////////////////////////////

  if (dim != n) throw exception("SparseBGMatrix::mv(): not conformable");
  //memset(result,'\0',sizeof(BG)*n);
  initial_BGarray(result,n,0.0);
...
SparseBGMatrix operator*(const SparseBGMatrix& A, const BG s)
{
   ...
   SparseBGMatrix B(new_dim,A.max_nz);
   B.nonzero = new_nz;
...
void SparseBGMatrix::release(void)
{
...
void SparseBGMatrix::release_nodestuff(void)
{
   node_list.clear();
   mask.clear();
   order.clear();
 ...
 */
BG SparseBGMatrix::q(const Vector<BG>& v1,const Vector<BG>& v2)
{
   if (dim == 0) throw exception("SparseBGMatrix is empty");
   if (dim != v1.size() || dim != v2.size()) throw exception("SparseBGMatrix::q(v1,v2): size not conformable");
 ...
 *
 * SparseBGMatrix A is half storage (upper or lower or mixed)
 * REQUIREMENTS: x and y must have at least this->dim elements
 *               x[0] and y[0] are the first elements
*/
BG SparseBGMatrix::q(const BG* x,const BG* y)
{
  if (dim == 0) exception("SparseBGMatrix is empty");
 ...
 *               x[0] and y[0] are the first elements.
 */
BG SparseBGMatrix::q(const BG* x,const BG* y,const int i1,const int i2)
{
  if (dim == 0) throw exception("SparseBGMatrix::q(): SparseBGMatrix is empty");
  if (i1<=0 || i1>dim || i2<=0 || i2>dim || i1>i2) throw exception("SparseBGMatrix::q(): out of range");
 ...
 *  SparseBGMatrix A is half storage only(upper,lower or mixed)
 */
BG SparseBGMatrix::qTLW(const Vector<BG>& v1,const Vector<BG>& v2)
{
   if (dim == 0) throw exception("SparseBGMatrix::qTLW(): it is empty");
   if (dim != v1.size() || dim != v2.size()) throw exception("SparseBGMatrix::qTLW(): size not conformable");
   return q(v1.begin(),v2.begin());
}
 ...
 *                 x[0] and y[0] are the first elements
 */
BG SparseBGMatrix::qTLW(const BG* x,const BG* y)
{
   if (dim ==0) throw exception("SparseBGMatrix::qTLW(): SparseBGMatrix is empty");
   if (!insertend) throw exception("SparseBGMatrix::qTLW(): first call SparseBGMatrix::close() first");
 ...
 *                x[0] and y[0] are the first elements
 */
BG SparseBGMatrix::qTLW(const BG* x,const BG* y,const int i1,
                  const int i2)
{
   if (dim == 0) throw exception("SparseBGMatrix::qTLW(): SparseBGMatrix is empty");
   if (!insertend) throw exception("SparseBGMatrix::qTLW(): call SparseBGMatrix::close() first");
   if (i1<=0 || i1>dim || i2<=0 || i2>dim || i1>i2) throw exception("SparseBGMatrix::qTLW(), out of range");
 ...
 */
void SparseBGMatrix::add_diag(const int ibeg,const int iend,const BG r)
{
   int   *ia = iiv-1;
   int   *ja = jjv-1;
   BG *a = aav-1;
 ...
 */
int SparseBGMatrix::solve(Vector<BG>& x,const Vector<BG>& b,const std::string &method,
                           double relax, double stopval,int mxiter)
{
   if (dim != b.size()) throw exception("SparseBGMatrix::solve(): size not conformable");
   if (x.size() < dim ) x.reserve(dim);
   if (method == "ysmp" || method == "ysmp1") {
      if (!factor_done && !factor()) throw exception("SparseBGMatrix::solve- returned with error from factor");
      x = solv(b);
      return 1;
   }
   else if (method == "ioc") {
      close();
      unsigned ir=0;
      //memset(x.begin(),'\0',sizeof(BG)*dim);
      x.resize(dim,0.0);
      gs_ioc(iiv-1,jjv-1,aav-1,b.begin()-1,x.begin()-1,dim,relax,stopval,mxiter,SESSION.epsilon,ir);
      if (!ir) throw exception("SparseBGMatrix::solve(): not converged");
      return 1;
   }
   else if (method == "iod") {
      throw exception("SparseBGMatrix::solve(): not available yet");
   }
   else {
      throw exception("SparseBGMatrix::solve(): no such solver");
  }
  return 0;
}
 ...
 */
int SparseBGMatrix::solve(BG* x, const BG* b,const std::string &method,
                           double relax, double stopval,int mxiter)
{
   if (method == "ysmp" || method == "ysmp1") {
      if (!factor_done && !factor()) throw exception("SparseBGMatrix::solve() returned with error from factor");
      ...
      close();
      unsigned ir=0;
      //memset(x,'\0',sizeof(BG)*dim);
      initial_BGarray(x,dim,0.0);
      gs_ioc(iiv-1,jjv-1,aav-1,b-1,x-1,dim,relax,stopval,
               mxiter,SESSION.epsilon,ir);
      if (!ir) warning("SparseBGMatrix::solve(): not converged");
      return 1;
    }
    else if (method == "iod") {
      throw exception("SparseBGMatrix::solve(): solver not available yet");
   }
   else {
      throw exception("SparseBGMatrix::solve():no such solver");
   }
 ...
 * </UL>
 */
void SparseBGMatrix::reorder(const int path)
{
   if (path ==1) {
 ...
 * </UL>
 */
unsigned SparseBGMatrix::factorization(const int path,const double tol)
{
   if (path < 4) throw exception("SparseBGMatrix::factorization(path): path >= 4 is required");
...
BG SparseBGMatrix::logdet(unsigned &irank)  {
   if (dim == 0) throw exception("SparseBGMatrix::logdet(): SparseBGMatrix is empty");
  unsigned i;
  BG res=0.0;
   if (!factor_done) {
 ...
 */
int SparseBGMatrix::insert(const int i,const int j,const BG value)
{
  int ii,jj; 
  ii = i;    
  jj = j;    
  
  if (dim == 0) throw exception("SparseBGMatrix::insert(): SparseBGMatrix is empty");
  if (i<=0 || j<=0 || j>dim) {
    std::cout << "insert "<< i<<","<<j;
    throw exception("SparseBGMatrix::insert(i,j,a): range error"); 
  }
  if (i>j) {
  ...
  //    if (i>j) throw exception("SparseBGMatrix::insert(i,j,a): i>j, only upper triangular allowed");
  ...
  throw exception("SparseBGMatrix::insert(), max_nz is too small");
  return(0);
}
 ...
 */
BG SparseBGMatrix::getaij(const int ii,const int jj)
{
  int i,j;
  i=ii;
  j=jj;
...
//throw exception("SparseBGMatrix::insert(i,j,a): i>j, only upper triangular allowed");
   if (j>dim) {
     //std::cout << "getaij "<< i<<","<<j;
    throw exception("SparseBGMatrix::getaij(i,j): range error"); 
  }
   ...
   throw exception("SparseBGMatrix::getaij This should not happen! Probably a bug :-( ");
   return(0.0);
}
 ...
 */
int SparseBGMatrix::reset(const int ii,const int jj,const BG value)
{
  int i=ii;
  int j=jj;

   if ( dim == 0 ) throw exception("SparseBGMatrix::reset(): SparseBGMatrix is empty");
   if (i<=0 || j<=0 ) throw exception("SparseBGMatrix::reset(i,j,a), range error");
   if ( i>j){
     i=jj;
     j=ii;
     //throw exception("SparseBGMatrix::reset(i,j,a): i<j only upper triangular allowed");
   }
   ...
   throw exception("SparseBGMatrix::insert(): max_nz too small");
   return(0);
}
 ...
 */
unsigned SparseBGMatrix::close(void)
{                      /* replace iiv by starting elements of rows:*/
   ...
   if (nrow != dim) {
      throw exception("SparseBGMatrix::close(): actual size <. expectec size");
   ...
   if (zero_pivot) {
      warning("zero-diagonals is found in a SparseBGMatrix, 1.0 has been added");
   }
   insertend = 1;
 ...
 */
doubleMatrix SparseBGMatrix::dense(unsigned r1, unsigned r2, unsigned c1, unsigned c2)  const
{
  ...
  for (k=1; k<= hsize; k++) {
	...
	if (i!=j) {
	  if (i>=r1 && i<=r2 && j>=c1 && j<=c2 ) {
	    out(i-r1+1,j-c1+1) = a[k].f;
	  }
          if (j>=r1 && j<=r2 && i>=c1 && i<=c2 ) {
	    out(j-r1+1,i-c1+1) = a[k].f;
	  }
	}
	else {
	  if (i>=r1 && i<=r2) {
	   out(i-r1+1,i-r1+1) = a[k].f;
	  }
	}
    }
  }
 ...
 * Display the content.
 */
void SparseBGMatrix::display(unsigned r1, unsigned r2, unsigned c1, unsigned c2, const std::string &msg)
{
  if (msg != "") std::cout <<"\n     " << msg << "\n";
  if (dim == 0) {
 ...
 */
void SparseBGMatrix::save(const std::string &fname, const int io_mode) const
{

   std::ofstream ofs(fname.c_str(),(OpenModeType)io_mode);
   if (!ofs) throw exception(std::string("SparseBGMatrix::save(): cannot open file:") + fname);
...
ostream& operator<<(ostream& stream, const SparseBGMatrix& A)
{
  unsigned dim = A.nrow();
   Matrix out;
...
static int squeeze(unsigned len, int *ia, int *ja, BG *a)
{
   unsigned lmar = 1;
   unsigned rmar = len;
   for (;;) {
 ...
 */
static void hsort_ija(unsigned n,int *ia,int *ja,BG *a)
{
...
int SparseBGMatrix::initialize_node_list(void) {
  int *ia   = iiv-1;
  int *ja   = jjv-1;
  BG *a = aav-1;
  unsigned i,j;
...
int SparseBGMatrix::minfilsymelm_node_list(void)
{
  unsigned i,j;
  ...
  for (i=1;i<=dim;i++) {
    ...
    qsort(num,deg,sizeof(unsigned), (cmp_func) comp1);
  }
...
#ifndef NEW_ELIMNODE
int SparseBGMatrix::elim_node(unsigned i, Vector<unsigned> Aorder)
{
  ...
  BGNodeStruct *elimnode, *nabornode;
...
int SparseBGMatrix::initial_lij(void)
{
  if (insertend) throw exception("SparseBGMatrix::initial_lij: cannot initial_lij after closing");
  unsigned i,j;
  unsigned row,col;
  unsigned deg,nabor;
  BG aij;
...
int SparseBGMatrix::factor(void){
  ...
  BG   *ptemp        = temp.begin();       ptemp--;
  BG   *pdiag        = diag.begin();       pdiag--;
  ...
  //memset(temp.begin(),'\0',dim*sizeof(BG));
  temp.resize(dim,0.0);
  ...
  for (i=1;i<=dim;i++){
    ...
    //         std::cout << std::endl;
    if (diagi > std::max(1.e-8,1.e-10*pdiag[coli].f)) { 
      info_vec[0]+=log(diagi);
      info_vec[1]+= 1.0;
    ...
    else if (diagi > -std::max(1.e-10,1.e-10*pdiag[coli].f)) {
      pdiag[coli] = 0;
      degi = node->adj_list.size();
      for (k=0;k<degi;k++){
	rowk = prows[k];
	...
	ptemp[rowk] = 0.0;
      ...
      throw exception("SparseBGMatrix::factor matrix is not positive semi-definite");
    }
  }
...
Vector<BG> SparseBGMatrix::solv(Vector<BG> rhs)
{
  if (!factor_done) throw exception("SparseBGMatrix::factor must be called first");
  unsigned i,j;
  ...
  BG   *pdiag  = diag.begin();  pdiag--;
  BG   *psol   = sol.begin();    psol--;
  BG   *pfsol  = fsol.begin();   pfsol--;
  BG   *prhs   = rhs.begin();    prhs--;
  unsigned *porder = order.begin(); porder--;
...
int SparseBGMatrix::solv(BG* x, const BG *rhs)
{
  if (!factor_done) throw exception("SparseBGMatrix::factor must be called first");
  unsigned i,j;
  unsigned coli,rowj;
  ...
  memcpy(temp_rhs.begin(),rhs,sizeof(BG)*dim);

  BG   *pdiag  = diag.begin();     pdiag--;
  BG   *psol   = sol.begin();       psol--;
  unsigned *porder = order.begin();    porder--;
  BG   *prhs   = temp_rhs.begin();  prhs--;
  BG   *pfsol; 
  pfsol = x; pfsol--;
...
void SparseBGMatrix::display_node_list(void) {
...
void SparseBGMatrix::display_lij(void) {
  // displays the lij's
  unsigned i,j;
  unsigned count;
  ...
  for (i=1;i<=dim;i++) {
    ...
    count = node_list(i).adj_list.size();
    for (j=1;j<=count;j++){
      std::cout << node_list(i).a(j).f <<" ";
    }
  }
  std::cout << std::endl;
}
void SparseBGMatrix::display_order(void) {
...
void SparseBGMatrix::display_inv_order(void) {
  // displays the order of diagonal nodes
  unsigned i;
  std::cout << "\n" << "inv_order" << "\n";
  for (i=1;i<=dim;i++) {
...
int SparseBGMatrix::solvrow(BG* x, const BG *rhs,unsigned row,Vector<BG> &sol,Vector<BG> &temp_rhs,unsigned *row_pt,unsigned *col_pt) {
  ...
  if (!factor_done) {
    throw exception("SparseBGMatrix::factor must be called first");
    return 0;
  }
  //  sol.reserve(dim);
  //  temp_rhs.reserve(dim);
  ...
  memcpy(temp_rhs.begin(),rhs,dim*sizeof(BG));
  allzero=1;
  BG   *pdiag  = diag.begin();     pdiag--;
  BG   *psol   = sol.begin();       psol--;
  unsigned *porder = order.begin();    porder--;
